...
// const utton = document.querySelector("#eLogin")
...
const setupPosts = data => {
    if(data.length){
        ...
        data.forEach(doc => {
          ...
          postList.innerHTML = '<p class="text-center">Hola Bienvenido a DrogonServers, Inicie sesión nuevamente.</p>';
...
//   rootReff.orderByChild("email").startAt(emailr).on('child_added', snap => {
//     console.log(snap.val(), snap.key);
...
// rootReff.orderByChild("email").endAt(25).once('value',  function(snapshot){
//   console.log(snapshot.val());
// });
